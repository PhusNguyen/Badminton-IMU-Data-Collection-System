/* Inlucde all the necessary libraries */
#include <Arduino.h>
...
void loop()
{
    /* Check if the server was available */
    if (!client.connect(host, port))
    {
        // Serial.println("Connection to server failed");
        // delay(1000); // Wait before retrying
        return; // Exit the loop if connection fails
    }

    /* When the server is available */
    /* Read 1 megaData every 10ms 10 times consecutively */
